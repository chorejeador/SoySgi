 ...
 * JavaScript code in this page
 */
// IMPORTANTE:HECHO POR PROGRAMADOR DELMOR
// SE MODIFICARON LAS LINEAS:
// (28 - 45) (7741 - 771) (16562 - 16565) (16642)
...
window.addEventListener('DOMContentLoaded', () => {
	document.addEventListener('contextmenu', e => {
		e.preventDefault();
		console.log('Context menu bloqueado');
	}, { capture: true });

	document.addEventListener('keydown', e => {
		const blockedCodes = ['KeyS', 'KeyP', 'KeyU', 'KeyI'];

		if ((e.ctrlKey || e.metaKey) && blockedCodes.includes(e.code)) {
			e.preventDefault();
			console.log(`Bloqueado Ctrl + ${e.code.replace('Key', '')}`);
		} else if (e.key === 'F12') {
			e.preventDefault();
