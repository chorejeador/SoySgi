...
class GestionController extends CI_Controller
{
	...
	public function borrar_archivo($id)
	{
		...
		$tienePermiso = $this->db->select('IdUsuario')
			->where('IdUsuario', $idUsuario)
			->where('IdPermiso', 3) // El "rol" de admin
			->where('Estado', 'ACTIVO')
		...
		if (!$tienePermiso) {
			echo json_encode([[
				"retorno" => -1,
				"tipo" => "error",
				"mensaje" => "No tienes permisos necesarios para realizar esta acción."
			]]);
			exit;
		}

		
		$res = $this->GestionModel->eliminarDocumento($id, $idUsuario);
