			...
			<div class="col-12">
				...
				<nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
					<ol class="breadcrumb pt-0">						
						<li class="breadcrumb-item"><a href="<?php echo base_url('index.php/gestiones');?>">Lista de Sub Gestiónes</a></li>
					</ol>
				</nav>
                    ...
                    <div class="card-body">
                        <form class="needs-validation tooltip-label-right" novalidate="">
                            <div class="form-group position-relative error-l-50">
								...
								<input type="text" readonly class="form-control" value="<?php echo $datos[0]["Descripcion"]; ?>">
								...
								<br>
								<label>Siglas: *</label> 
								<input type="text" id="siglas" class="form-control" required="" placeholder="Ejem. E 01">
                                ...
                                <?php 
                                    if ($idProcesoParam == null) {
										echo '<option label="Seleccione a la gestión al que esta sub gestión pertenece:" disabled selected></option>';
									} ?>
									<?php
										foreach ($datos as $key ) {
											if ($idProcesoParam != null) {
												if ($idProcesoParam == $key["IdGestion"]) {
													echo '<option value="'.$key["IdGestion"].'" selected>'.$key["Descripcion"].'</option>';
												}
											}else{
												echo '<option value="'.$key["IdGestion"].'">'.$key["Descripcion"].'</option>';
											}
										}
									?>
                                </select>
